...
const SidebarNavList = (props) => {
  ...
  var highlight = props.data.highlight && (
    <i className="fa fa-paper-plane text-theme"></i>
  );
  var title = props.data.title && (
    <div className="menu-text">{props.data.title} {label} {highlight}</div>
  );
              ...
              {props.data.children
                ? (
                  <Link
                    ...
                    className="menu-link"
                    onMouseOver={(e) =>
                      context.handleAppSidebarOnMouseOver(e, props.data)}
                    onMouseOut={(e) =>
                      context.handleAppSidebarOnMouseOut(e, props.data)}
              ...
              {props.data.children && (
                <div
                  className={"menu-submenu " +
                    (((props.active || (props.clicked === -1 && match) ||
                        props.data.search) && !context.appSidebarMinified)
                      ? "d-block "
                      : "d-none")}
                >
                  {props.data.children &&
