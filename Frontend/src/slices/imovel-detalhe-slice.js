...
export const createImovelDetalhe = createAsyncThunk(
  ...
  async ({ data }) => {
    return await ImovelDetalheService.create(data);
  }
);
...
export const listAllImovelDetalhe = createAsyncThunk(
  ...
  async () => {
    return await ImovelDetalheService.getAll();
  }
);
...
export const listImovelDetalhe = createAsyncThunk(
  "imoveldetalhe/get",
  async ({ id }) => {
    await ImovelDetalheService.get(id);
    return { id };
  }
);
...
export const listImovelWithDetalhes = createAsyncThunk(
  ...
  async ({ id }) => {
    return await ImovelService.getWithDetalhes(id);
  }
);
...
export const updateImovelDetalhe = createAsyncThunk(
  ...
  async ({ id, data }) => {
    return await ImovelDetalheService.update(id, data);
  }
);
...
export const deleteImovelDetalhe = createAsyncThunk(
  ...
  async ({ id }) => {
    await ImovelDetalheService.remove(id);
    return { id };
  }
);
  ...
  extraReducers: {
    ...
    [updateImovelDetalhe.fulfilled]: (state, action) => {
      const index = state.findIndex(
        imovel_detalhe => imovel_detalhe.id === action.payload.id
      );
      state[index] = {
        ...state[index],
        ...action.payload
